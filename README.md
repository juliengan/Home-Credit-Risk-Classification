...
in our MLproject file we have an entrypoint for each step of our pipelines:
    
    |-  entrypoints
           ├── create_pipeline       <- Launches the src/features/pipeline.py script to set up the pipeline (the pipeline is saved into models/pipe.pkl)
           ├── data_prep             <- Launches the src/features/build_features.py script that apply our pipeline on the raw datasets (train and test) (results are stored into data/processed)
...
We used airflow to orchestrated our pipelines.
We have to dags on airflow. Each of them has bash operators that will just launch specifics mlflow entrypoints in a sequential way
